...
export const SCANLINE_OPACITY = 0.55;

export function updateSvgScanline(filePath, opacity = SCANLINE_OPACITY) {
  if (!fs.existsSync(filePath)) return false;
  let content = fs.readFileSync(filePath, "utf8");

  // Update @keyframes scanlineAnim opacity
  content = content.replace(
    /@keyframes scanlineAnim\s*\{[\s\S]*?0%\s*\{[^}]*\}[\s\S]*?50%\s*\{[^}]*\}[\s\S]*?100%\s*\{[^}]*\}\s*\}/,
    `@keyframes scanlineAnim {\n      0%   { transform: translateY(35px); opacity: ${opacity}; }\n      50%  { transform: translateY(328px); opacity: ${opacity}; }\n      100% { transform: translateY(620px); opacity: ${opacity}; }\n    }`
  );

  // Update .scan-laser rule
  content = content.replace(
    /(\.scan-laser\s*\{[\s\S]*?animation:\s*scanlineAnim[^;]+;[\s\S]*?filter:[^;]+;\s*)(?:opacity:[^;]+;\s*)?(\})/,
    `$1opacity: ${opacity};\n    $2`
  );

  // Update <line class="scan-laser" ... opacity="..."
  content = content.replace(
    /(<line\s+class="scan-laser"[^>]*?\bopacity=")[^"]*(")/,
    `$1${opacity}$2`
  );
...
export function updateAllCards(opacity = SCANLINE_OPACITY) {
  const rootDir = path.resolve(".");
  const darkPath = path.join(rootDir, "dark.svg");
  const lightPath = path.join(rootDir, "light.svg");
  let updated = 0;
  if (updateSvgScanline(darkPath, opacity)) {
    console.log(`Updated dark.svg scanline opacity to ${opacity}`);
    updated++;
  }
  if (updateSvgScanline(lightPath, opacity)) {
    console.log(`Updated light.svg scanline opacity to ${opacity}`);
    updated++;
  }
  return updated;
}
...
async function main() {
  updateAllCards(SCANLINE_OPACITY);

  if (!USERNAME || !TOKEN) {
    if (!USERNAME) console.log("Notice: GH_USERNAME not set (skipped jet heatmap generation).");
    if (!TOKEN) console.log("Notice: GH_TOKEN / GITHUB_TOKEN not set (skipped jet heatmap generation).");
    return;
  }
